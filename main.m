...
% ------------------------------------------------------------------------
% Save Box-Behnken Design as its OWN separate Excel workbook
% ------------------------------------------------------------------------
BBD_ExcelFileName = "BBD_Design_Matrix.xlsx";
if isfile(BBD_ExcelFileName)
    delete(BBD_ExcelFileName);
end
writetable(BBD_CodedTable,  BBD_ExcelFileName, "Sheet","BBD_Coded_Design");
writetable(BBD_ActualTable, BBD_ExcelFileName, "Sheet","BBD_Simulation_Matrix");
fprintf("Box-Behnken design also saved separately to: %s\n\n", BBD_ExcelFileName);
...
% ------------------------------------------------------------------------
% Save 1000-point candidate space as its OWN separate Excel workbook
% ------------------------------------------------------------------------
Candidate_ExcelFileName = "Candidate_1000_Points.xlsx";
if isfile(Candidate_ExcelFileName)
    delete(Candidate_ExcelFileName);
end
writetable(CandidateResults, Candidate_ExcelFileName, "Sheet","Candidate_1000_Points");
fprintf("1000-point candidate dataset also saved separately to: %s\n\n", Candidate_ExcelFileName);
...
%% ============================================================
%% STEP 12.5 — PUBLICATION TABLES (Table 1 - Table 9)
%% ============================================================
fprintf("\n============================================================\n");
fprintf(" STEP 12.5 — BUILDING AND SAVING PUBLICATION TABLES (Table 1-9)\n");
fprintf("============================================================\n");

% ------------------------------------------------------------------------
% Table 1 — Material Properties and Workpiece Specifications of
...
Table1_Value = [ ...
    ...
    Diameter*1000; ...
    Length*1000];
Table1_MaterialProperties = table(Table1_Property, Table1_Value, ...
    'VariableNames', {'Property','Value'});
fprintf("\n---- Table 1: Material Properties and Workpiece Specifications of 17-4 PH Stainless Steel ----\n");
disp(Table1_MaterialProperties);
writecell({"Table 1. Material Properties and Workpiece Specifications of 17-4 PH Stainless Steel"}, ...
    ExcelFileName, "Sheet","Table1_Material_Properties", "Range","A1");
writetable(Table1_MaterialProperties, ExcelFileName, "Sheet","Table1_Material_Properties", "Range","A3");

% ------------------------------------------------------------------------
% Table 2 — Turning Parameters and Their Levels
% ------------------------------------------------------------------------
Table2_Parameter = ["Cutting Speed (Vc)"; "Feed Rate (f)"; "Depth of Cut (ap)"];
Table2_Symbol    = ["Vc"; "f"; "ap"];
Table2_Unit      = ["m/min"; "mm/rev"; "mm"];
Table2_Low    = [TurningParams.Vc.Low;    TurningParams.f.Low;    TurningParams.ap.Low];
Table2_Center = [TurningParams.Vc.Center; TurningParams.f.Center; TurningParams.ap.Center];
Table2_High   = [TurningParams.Vc.High;   TurningParams.f.High;   TurningParams.ap.High];
Table2_TurningParameters = table(Table2_Parameter, Table2_Symbol, Table2_Unit, ...
    Table2_Low, Table2_Center, Table2_High, ...
    'VariableNames', {'Parameter','Symbol','Unit','Low','Center','High'});
fprintf("\n---- Table 2: Turning Parameters and Their Levels ----\n");
disp(Table2_TurningParameters);
writecell({"Table 2. Turning Parameters and Their Levels"}, ...
    ExcelFileName, "Sheet","Table2_Turning_Parameters", "Range","A1");
writetable(Table2_TurningParameters, ExcelFileName, "Sheet","Table2_Turning_Parameters", "Range","A3");

% ------------------------------------------------------------------------
% Table 3 — Box-Behnken Numerical Simulation Matrix (17 runs)
% ------------------------------------------------------------------------
Table3_BBDMatrix = BBD_ActualTable;
fprintf("\n---- Table 3: Box-Behnken Numerical Simulation Matrix ----\n");
disp(Table3_BBDMatrix);
writecell({"Table 3. Box-Behnken Numerical Simulation Matrix"}, ...
    ExcelFileName, "Sheet","Table3_BBD_Simulation_Matrix", "Range","A1");
writetable(Table3_BBDMatrix, ExcelFileName, "Sheet","Table3_BBD_Simulation_Matrix", "Range","A3");

% ------------------------------------------------------------------------
% Table 4 — Numerical Simulation and Machining Response Results
% ------------------------------------------------------------------------
Table4_NumericalResults = TurningSimResults(:, {'Run','Vc_m_per_min','f_mm_per_rev','ap_mm', ...
    'CuttingForce_N','CuttingPower_W','MaxFieldTemp_C','MaxVonMisesStress_MPa', ...
    'MaxDisplacement_mm','Ra_um','ToolWear_VB_mm','MRR_mm3_per_min','Energy_Wh'});
Table4_NumericalResults.Properties.VariableNames = { ...
    'Run','Vc_m_per_min','f_mm_per_rev','ap_mm','Fc_N','Pc_W','Tmax_C', ...
    'SigmaMax_MPa','Displacement_mm','Ra_um','VB_mm','MRR_mm3_per_min','Energy_Wh'};
fprintf("\n---- Table 4: Numerical Simulation and Machining Response Results ----\n");
disp(Table4_NumericalResults);
writecell({"Table 4. Numerical Simulation and Machining Response Results"}, ...
    ExcelFileName, "Sheet","Table4_Numerical_Results", "Range","A1");
writetable(Table4_NumericalResults, ExcelFileName, "Sheet","Table4_Numerical_Results", "Range","A3");

% ------------------------------------------------------------------------
% Table 5 — RSM Model Performance for Machining Responses
% ------------------------------------------------------------------------
Table5_RSMPerformance = RSM_Summary(:, {'Response','R2','AdjR2','PredR2','RMSE','MAE','p_LOF'});
Table5_RSMPerformance.Properties.VariableNames = { ...
    'Response','R2','AdjustedR2','PredictedR2','RMSE','MAE','LOF_pValue'};
fprintf("\n---- Table 5: RSM Model Performance for Machining Responses ----\n");
disp(Table5_RSMPerformance);
writecell({"Table 5. RSM Model Performance for Machining Responses"}, ...
    ExcelFileName, "Sheet","Table5_RSM_Performance", "Range","A1");
writetable(Table5_RSMPerformance, ExcelFileName, "Sheet","Table5_RSM_Performance", "Range","A3");

% ------------------------------------------------------------------------
% Table 6 — NSGA-II Optimization Parameters and Objectives
% ------------------------------------------------------------------------
Table6_Setting = [ ...
    "Population Size"; ...
    "Max Generations"; ...
    ...
    "ap Bounds (mm)"];
Table6_Value = [ ...
    string(gaOpts.PopulationSize); ...
    string(gaOpts.MaxGenerations); ...
    string(gaOpts.ParetoFraction); ...
    "4"; ...
    "Minimize Surface Roughness, Ra"; ...
    "Minimize Tool Wear, VB"; ...
    "Maximize Material Removal Rate, MRR"; ...
    "Minimize Energy Consumption, Energy"; ...
    sprintf("[%.0f, %.0f]", TurningParams.Vc.Low, TurningParams.Vc.High); ...
    sprintf("[%.2f, %.2f]", TurningParams.f.Low,  TurningParams.f.High); ...
    sprintf("[%.2f, %.2f]", TurningParams.ap.Low, TurningParams.ap.High)];
Table6_NSGA2Settings = table(Table6_Setting, Table6_Value, ...
    'VariableNames', {'Parameter','Value'});
fprintf("\n---- Table 6: NSGA-II Optimization Parameters and Objectives ----\n");
disp(Table6_NSGA2Settings);
writecell({"Table 6. NSGA-II Optimization Parameters and Objectives"}, ...
    ExcelFileName, "Sheet","Table6_NSGA2_Settings", "Range","A1");
writetable(Table6_NSGA2Settings, ExcelFileName, "Sheet","Table6_NSGA2_Settings", "Range","A3");

% ------------------------------------------------------------------------
% Table 7 — Comparison of Multi-Objective Optimization Methods
% ------------------------------------------------------------------------
Table7_OptimizationComparison = OptimizationComparison;
fprintf("\n---- Table 7: Comparison of Multi-Objective Optimization Methods ----\n");
disp(Table7_OptimizationComparison);
writecell({"Table 7. Comparison of Multi-Objective Optimization Methods"}, ...
    ExcelFileName, "Sheet","Table7_Optimization_Comparison", "Range","A1");
writetable(Table7_OptimizationComparison, ExcelFileName, "Sheet","Table7_Optimization_Comparison", "Range","A3");

% ------------------------------------------------------------------------
% Table 8 — Sensitivity of Turning Parameters to Machining Responses
% ------------------------------------------------------------------------
Table8_Sensitivity = SensitivityTableXL;
fprintf("\n---- Table 8: Sensitivity of Turning Parameters to Machining Responses ----\n");
disp(Table8_Sensitivity);
writecell({"Table 8. Sensitivity of Turning Parameters to Machining Responses (% Contribution)"}, ...
    ExcelFileName, "Sheet","Table8_Sensitivity", "Range","A1");
writetable(Table8_Sensitivity, ExcelFileName, "Sheet","Table8_Sensitivity", "Range","A3");

% ------------------------------------------------------------------------
% Table 9 — Numerical Confirmation of the Selected Optimal Solution
% ------------------------------------------------------------------------
Table9_Confirmation = ConfirmationTable;
fprintf("\n---- Table 9: Numerical Confirmation of the Selected Optimal Solution ----\n");
disp(Table9_Confirmation);
writecell({"Table 9. Numerical Confirmation of the Selected Optimal Solution"}, ...
    ExcelFileName, "Sheet","Table9_Confirmation", "Range","A1");
writetable(Table9_Confirmation, ExcelFileName, "Sheet","Table9_Confirmation", "Range","A3");

fprintf("\nAll 9 publication tables saved to: %s\n", ExcelFileName);
fprintf("(Sheets: Table1_Material_Properties ... Table9_Confirmation)\n\n");
